...
export default function PokemonList() {
    ...
    const [listOfPokemons, setListPoke] = useState<Pokemon[]>([
      ...
      function onSubmitP(event: React.FormEvent<HTMLFormElement>){
        ...
        function deleteNameP(pokeName: string) {
          setListPoke(listOfPokemons.filter((poke) => poke.name !== pokeName))
        ...
        <div className="grid-pokemon">
            {
                listOfPokemons.map(poke => {
                    return <PokemonBox key={poke.name} {...poke} onDelete={() => deleteNameP(poke.name)} />
                })
            }
        </div>
    </div>)
